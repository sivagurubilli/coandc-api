...
exports.checkWhatsappOtpValidation = async (req, res, next) => {
    try {
        const schema = Joi.object({
            ...
            otp: Joi.string().trim().pattern(/^\d+$/).min(4).max(4).messages({
                'string.pattern.base': 'Please enter a valid otp',
                'string.empty': 'Otp is required',
                'string.min': 'Otp must have exactly 4 digits',
                'string.max': 'Otp must have exactly 4 digits'
            })
        }).unknown(false)
            .messages({

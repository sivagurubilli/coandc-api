...
exports.checkCreateEmployerPlanValidation = async (req, res, next) => {
    try {
        const schema = Joi.object({
            ...
            employerPlanName: Joi.string().required().messages({
            ...
            assistancePrice: Joi.when('supportAssistance', {
                ...
                then: Joi.number().integer().min(1).required().messages({
                    ...
                    'number.min': 'assistancePrice must greater than 0'
                }),
                otherwise: Joi.forbidden(),
            }).messages({
                'any.unknown': 'assistancePrice is not allowed in payload when supportAssistance is 1',
            }),
